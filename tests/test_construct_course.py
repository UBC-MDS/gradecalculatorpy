...
def test_construct_course():
    ...
    assert sum(course_df["Weights (%)"]) == 100, "Course components weights not adding up to 100%"

    # Make sure generated .csv has third column "Grades (%)" with all NANs
    assert course_df["Grades (%)"].isnull().all(), "Course components grades should all be NAN for now"
